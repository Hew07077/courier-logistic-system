...
public class TrackOrderPanel extends JPanel {
    ...
    private void showWelcomeMessage() {
        ...
        System.out.println("TrackOrderPanel - User email: " + userEmail + ", orders found: " + userOrders.size());
    ...
    private JPanel createOrderCard(SenderOrder order) {
        ...
        JLabel dateLabel = new JLabel(order.getOrderDate());
    ...
    private void displayTrackingResult(String trackingNumber) {
        ...
        if (currentOrder == null) {
            ...
            if (currentOrder.getCustomerEmail() != null && userEmail != null) {
                if (currentOrder.getCustomerEmail().trim().equalsIgnoreCase(userEmail.trim())) {
                    isAuthorized = true;
                }
            }
    ...
    private void displayOrderDetails(SenderOrder order) {
    ...
    private JPanel createClickableHeader(SenderOrder order) {
        JPanel headerPanel = new JPanel(new BorderLayout());
        ...
        JLabel headerLabel = new JLabel("📦 Order " + order.getId());
    ...
    private JPanel createStatusPanel(SenderOrder order) {
        ...
        JLabel statusLabel = new JLabel("Current Status: " + order.getStatus());
        statusLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
        statusLabel.setForeground(getStatusColor(order.getStatus()));
        statusInfo.add(statusLabel);
        
        JLabel updateLabel = new JLabel("Last updated: " + order.getOrderDate());
    ...
    private JPanel createAddressPanel(SenderOrder order) {
        ...
        JLabel fromAddress = new JLabel("<html>" + order.getCustomerAddress() + "</html>");
        ...
        JLabel toAddress = new JLabel("<html>" + order.getRecipientAddress() + "</html>");
    ...
    private JPanel createTimelinePanel(SenderOrder order) {
        ...
        timelinePanel.add(createTimelineEvent("Order Placed", order.getCustomerName(), order.getOrderDate(), true));
        ...
        if ("In Transit".equals(order.getStatus()) || "Delayed".equals(order.getStatus())) {
            String status = "In Transit";
            if ("Delayed".equals(order.getStatus())) {
                String reason = extractReason(order.getNotes());
                status = "Delayed - " + (reason != null ? reason : "Unknown reason");
            }
        ...
        if ("Delivered".equals(order.getStatus())) {
            timelinePanel.add(createTimelineEvent("Delivered", "Package delivered", 
                order.getEstimatedDelivery() != null ? order.getEstimatedDelivery() : "Completed", true));
        }
    ...
    private String extractReason(String notes) {
        if (notes != null && notes.contains("Reason:")) {
    ...
    private JPanel createPackagePanel(SenderOrder order) {
    ...
    private JButton createDetailsButton(SenderOrder order) {
    ...
    private void showFullOrderDetails(SenderOrder order) {
        ...
        JLabel iconLabel = new JLabel(getStatusIcon(order.getStatus()) + " ");
