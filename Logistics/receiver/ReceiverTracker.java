...
/**
 * Receiver Tracking System - Allows recipients to track their orders by Order ID
 */
public class ReceiverTracker extends JFrame {
    ...
    private ReceiverOrderData parseOrderData(String[] parts) {
        ...
        order.customerName = safeString(parts, 1);
        order.customerPhone = safeString(parts, 2);
        order.customerEmail = safeString(parts, 3);
        ...
        if (parts.length > 14 && !parts[14].isEmpty()) {
            try {
                order.weight = Double.parseDouble(parts[14]);
            } catch (NumberFormatException e) {}
        }
        
        order.dimensions = safeString(parts, 15);
        order.notes = safeString(parts, 16);
        order.reason = safeString(parts, 17);
        
        // Payment info
        if (parts.length > 27) {
            order.paymentStatus = safeString(parts, 27);
            order.paymentMethod = safeString(parts, 28);
            order.transactionId = safeString(parts, 29);
            order.paymentDate = safeString(parts, 30);
        }
    ...
    // Inner class for order data (renamed to avoid conflict)
