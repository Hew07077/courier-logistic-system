...
public class OrderManagement {
    ...
    private void assignDriver() {
        ...
        // If DriverManagement is available, show list of available drivers
        if (driverManagement != null) {
            List<logistics.driver.Driver> availableDrivers = driverManagement.getAvailableDrivers();
            if (!availableDrivers.isEmpty()) {
                String[] driverOptions = availableDrivers.stream()
                    .map(d -> d.id + " - " + d.name)
                    .toArray(String[]::new);
                ...
                if (selected != null) {
                    String driverId = selected.split(" - ")[0];
                    assignDriverToOrder(order, driverId);
                }
                return;
            }
        }
        ...
        if (driverId != null && !driverId.trim().isEmpty()) {
            assignDriverToOrder(order, driverId.trim());
        }
    }
    
    private void assignDriverToOrder(Order order, String driverId) {
        order.driverId = driverId;
        
        // Auto-assign vehicle based on driver (simplified)
